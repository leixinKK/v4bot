...
async def mychangeexport(event):
    try:
        ...
        async with jdbot.conversation(SENDER, timeout=60) as conv:
            ...
            btns0 = [
                Button.inline("修改此变量", data="change"),
                Button.inline('删除此变量', data='dele'),
                Button.inline('取消对话', data='quxiao')
            ]
            msg = await jdbot.edit_message(msg, f'这是{res}变量，对应的值为：\n```{valuedata}```\n请做出您的选择：', buttons=split_list(btns0, row))
            convdata0 = await conv.wait_event(press_event(int(chat_id)))
            res0 = bytes.decode(convdata0.data)
            if res0 == 'quxiao':
                await jdbot.edit_message(msg, '对话已取消，感谢你的使用')
                conv.cancel()
                return
            elif res0 == 'change':
                btns1 = [Button.inline("是", data=res), Button.inline("否", data="no")]
                msg = await jdbot.edit_message(msg, f"这是{res}变量对应的值：\n```{valuedata}```\n请问你需要修改吗？", buttons=split_list(btns1, row))
                convdata = await conv.wait_event(press_event(SENDER))
                res = bytes.decode(convdata.data)
                if res == 'no':
                    await jdbot.edit_message(msg, '对话已取消，感谢你的使用')
                ...
                btns2 = [Button.inline("是", data="yes"), Button.inline("否", data="no")]
                msg = await jdbot.edit_message(msg, f"这是{res}变量，对应的值为：\n```{valuedata}```\n请问你确定要删除此变量吗？", buttons=split_list(btns2, row))
                convdata = await conv.wait_event(press_event(SENDER))
